...
public class GraphUtility <Type> {
	...
	public static <Type> boolean areConnected(List<Type> sources, List<Type> destinations, Type srcData, Type dstData) {
		...
		Iterator it = graph.getVertice(srcData.toString()).edges();
		return recursiveConnected(it,graph,srcData,dstData);
	}

			public static <Type> boolean recursiveConnected(Iterator it, Graph graph, Type currentData, Type dstData){

			System.out.println(it.next().toString());
			while(it.hasNext()){
				Edge next = (Edge)it.next();
				if(next.getOtherVertex().getName()==dstData.toString()){
					return true;
				}
				if(next.getOtherVertex().getVisited() == false){
					next.getOtherVertex().isVisited(true) ;
				recursiveConnected(it,graph,next.getOtherVertex().getData(), dstData);
				}
